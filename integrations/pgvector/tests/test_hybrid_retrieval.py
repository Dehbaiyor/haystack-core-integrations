...
class TestHybridRetrieval:
    ...
    def test_hybrid_retrieval_cosine_similarity(self, document_store: PgvectorDocumentStore):
        ...
        results = document_store._hybrid_retrieval(
    ...
    @pytest.mark.parametrize("document_store", ["document_store", "document_store_w_hnsw_index"], indirect=True)
    def test_hybrid_retrieval_inner_product(self, document_store: PgvectorDocumentStore):
        query_embedding = [0.1] * 768
        most_similar_embedding = [0.8] * 768
        second_best_embedding = [0.8] * 700 + [0.1] * 3 + [0.2] * 65
        another_embedding = rand(768).tolist()
    ...
    @pytest.mark.parametrize("document_store", ["document_store", "document_store_w_hnsw_index"], indirect=True)
    def test_hybrid_retrieval_l2_distance(self, document_store: PgvectorDocumentStore):
        query_embedding = [0.1] * 768
    ...
    @pytest.mark.parametrize("document_store", ["document_store", "document_store_w_hnsw_index"], indirect=True)
    def test_hybrid_retrieval_with_filters(self, document_store: PgvectorDocumentStore):
        ...
        results = document_store._hybrid_retrieval(query=QUERY, query_embedding=query_embedding, top_k=3, filters=filters)
    ...
    def test_empty_query_embedding(self, document_store: PgvectorDocumentStore):
        ...
        with pytest.raises(ValueError):
            document_store._hybrid_retrieval(query=QUERY, query_embedding=query_embedding)
